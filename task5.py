...
async def perform_operation(identifier: str) -> ApplicationResponse:
    ...
    while (datetime.now() - start_time).total_seconds() < 15:
        f1 = asyncio.ensure_future(get_application_status1(identifier))
        f2 = asyncio.ensure_future(get_application_status2(identifier))
...
async def main():
    ...
    res = await perform_operation(identifier)
    print(res)
